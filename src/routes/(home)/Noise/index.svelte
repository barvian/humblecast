<script lang="ts">
	...
	$: if (mesh) {
		...
		mesh.program.uniforms.u_res.value = new Vec4(width, height, a1, a2)
	}

	const nMouse = new Vec2(-1),
		velocity = new Vec2(),
		lastNMouse = new Vec2()
	let lastTime: number
	// Handle mouse move
...
<div
	class="absolute inset-0 opacity-0 transition duration-1000"
...
>
	...
	<svg
		...
		class="absolute w-full h-full top-0 left-0 mix-blend-overlay opacity-80"
	>
		<defs>
