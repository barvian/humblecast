...
<svg {width} {height} viewBox="0 0 {width} {height}" class="sr-only">
	<filter id={filterId}>
		<feTurbulence type="fractalNoise" baseFrequency="0.003 0.003" numOctaves="5" seed="2" stitchTiles="noStitch" x="0%" y="0%" width="100%" height="100%" result="noise" />
...
<SvelteImg {...$$restProps} class="ease-out-expo transform {cls}" style="filter: url(#{filterId}); --tw-scale-y: {loaded ? 100 : scale*1.2}%; transition-property: opacity, transform; transition-duration: {duration}ms;" data-loaded={loaded ? '' : undefined} bind:ref on:load={() => { loaded = true }} />
