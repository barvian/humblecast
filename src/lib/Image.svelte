...
<SvelteImg class="ease-out-expo {cls}" {...$$restProps} style="filter: url(#{filterId}); transition-property: opacity, transform; transition-duration: {duration}ms, {duration*1.5}ms;" data-loaded={loaded ? '' : undefined} bind:ref on:load={() => { loaded = true }} />
